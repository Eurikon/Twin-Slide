...
extern Model *modelTwinSlideExpander;
extern Model *modelMonoSlide;
...
struct RefreshCounter {
	...
	unsigned int refreshCounter = (random::u32() % displayRefreshStepSkips);// stagger start values to avoid processing peaks when multiple module instances in the patch
